...
class CongTy:
    ...
    def xoa_nv_theo_ma_nv(self, ma_nv):
        """8. Tự xây dựng thêm các thao tác thích hợp cho bài toán (Xóa nhân viên theo mã nhân viên)"""
        nv_can_xoa = self.tim_nv_theo_ma_nv(ma_nv)

        if nv_can_xoa:
            self.ds.remove(nv_can_xoa)
...
if __name__ == '__main__':
    ...
    print("\n Test: 8. Tự xây dựng thêm các thao tác thích hợp cho bài toán (Xóa nhân viên theo mã nhân viên)")
    ct.xoa_nv_theo_ma_nv(456) #Có
    ct.xoa_nv_theo_ma_nv(368) #Không
